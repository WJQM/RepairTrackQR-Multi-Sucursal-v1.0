...
export async function POST(req: Request) {
  ...
  try {
    ...
    const item = await prisma.software.create({
      data: { name, category: category || null, minRequirements: minRequirements || null, recRequirements: recRequirements || null, size: size || null, image: image || null, branchId },
    });
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    const item = await prisma.software.update({
      where: { id },
      data: { ...(name !== undefined && { name }), ...(category !== undefined && { category }), ...(minRequirements !== undefined && { minRequirements }), ...(recRequirements !== undefined && { recRequirements }), ...(size !== undefined && { size }), ...(image !== undefined && { image }), ...(active !== undefined && { active }) },
    });
