...
export async function GET(req: Request) {
  ...
  try {
    ...
    const items = await prisma.inventoryItem.findMany({ where, include: { branch: { select: { id: true, name: true } } }, orderBy: { createdAt: "desc" } });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const item = await prisma.inventoryItem.create({
      data: { name, category: category || null, quantity: parseInt(quantity) || 0, price: parseFloat(price) || 0, minStock: parseInt(minStock) || 5, image: image || null, branchId },
    });
  ...
  try {
    ...
    const item = await prisma.inventoryItem.update({
      ...
      data: {
        ...
        ...(quantity !== undefined && { quantity: parseInt(quantity) }), ...(price !== undefined && { price: parseFloat(price) }),
        ...(minStock !== undefined && { minStock: parseInt(minStock) }), ...(image !== undefined && { image }),
