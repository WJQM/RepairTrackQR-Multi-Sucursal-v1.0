...
export default function QuotationsPage() {
  ...
  const saveQuotation = async () => {
    ...
    if (editingQuotation) {
      ...
      try {
        ...
        if (res.ok) { await loadQuotations(); setShowModal(false); sileo.success({ title: "Documento actualizado" }); }
      } catch {}
    } else {
      if (modalType === "sale") {
        const ok = await reduceStock(selectedItems);
      ...
      try {
        ...
        if (res.ok) { await loadQuotations(); setShowModal(false); sileo.success({ title: modalType === "quotation" ? "Cotización creada" : "Nota de venta registrada" }); }
      } catch {}
    }
  };
  ...
  const deleteQuotation = async (q: Quotation) => {
    ...
    try {
      ...
      if (res.ok) { await loadQuotations(); setViewQuotation(null); sileo.success({ title: "Documento eliminado" + (q.type === "sale" ? " y stock restaurado" : "") }); }
    } catch {}
  };
  ...
  const convertToSale = async (quotation: Quotation) => {
    ...
    try {
      ...
      if (res.ok) { await loadQuotations(); setViewQuotation(null); sileo.success({ title: "Cotización convertida a nota de venta" }); }
    } catch {}
  };
  ...
  const printQROnly = (q: Quotation) => {
    ...
    const qrUrl = `${origin}/quotations/print/${q.code}?branchId=${activeBranch}`;
    ...
    const qrImg = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(qrUrl)}&color=000000&bgcolor=ffffff&margin=0`;
    const w = window.open("", "_blank");
    if (!w) return;
    // ✅ MODIFICADO: Se eliminaron datos del cliente (nombre, teléfono, total) del popup QR
    w.document.write(`<!DOCTYPE html><html><head><title>${title} - ${q.code}</title>
...
{/* ═══ MODAL CREAR / EDITAR ═══ */}
      ...
      <aside className={`sidebar-desktop${menuOpen ? " open" : ""}`} style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 200, transition: "transform 0.3s ease", background: "rgba(12,12,18,0.95)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", zIndex: 45, padding: "0 10px" }}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflow: "auto", padding: "4px 0" }}>
          ...
          {[...(user?.role === "tech" ? [{ label: "Mis Asignaciones", path: "/asignaciones", icon: "📋" }, { label: "Escáner", path: "/scanner", icon: "📷" }, { label: "Cotizaciones", path: "/quotations", icon: "🧾" }] : [{ label: "Panel Principal", path: "/dashboard", icon: "📋" }, { label: "Servicios", path: "/services", icon: "🛠️" }, { label: "Inventario", path: "/inventory", icon: "📦" }, { label: "Software", path: "/software", icon: "🎮" }, { label: "Escáner", path: "/scanner", icon: "📷" }, { label: "Cotizaciones", path: "/quotations", icon: "🧾" }, { label: "Extracto", path: "/extracto", icon: "📊" }, ...(user?.role === "superadmin" ? [{ label: "Usuarios", path: "/admin/users", icon: "👥" }, { label: "Sucursales", path: "/admin/branches", icon: "🏢" }, { label: "Configuración", path: "/admin/settings", icon: "⚙️" }] : [])])].map(item => ({ ...item, active: item.path === "/quotations" })).map((item) => (
