...
function parseNotesAll(notesField: string | null, svcList: ServiceItem[]): { notes: string; services: string[]; software: string[]; repuestos: string[]; deliveryNotes: string; discount: string } {
  ...
  return { notes: notesParts.join(" | "), services, software, repuestos, deliveryNotes, discount };
}

function buildNotesString(obs: string, services: string[], software: string[], repuestos: string[], deliveryNotes: string, discount?: string): string {
  const parts: string[] = [];
...
export default function DashboardPage() {
  ...
  const confirmDelivery = async () => {
    ...
    try {
      ...
      const finalNotes = buildNotesString(originalObs, deliverySelectedServices, deliverySelectedSoftware, invNames, deliveryNotes, deliveryDiscount);
  ...
  const buildWhatsAppStatusMsg = (repair: Repair, newStatus: string) => {
    const st = STATUS[newStatus];
    const base = getBaseUrl();
    const trackUrl = `${base}/portal`;
    const lines = [
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalNotes = buildNotesString(editNotes, editSelectedServices, editSelectedSoftware, invNames, existingParsed.deliveryNotes, existingParsed.discount);
      ...
      <aside className={`sidebar-desktop${menuOpen ? " open" : ""}`} style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 200, transition: "transform 0.3s ease", background: "rgba(12,12,18,0.95)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", zIndex: 45, padding: "0 10px" }}>
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: "12px 6px" }}>
          ...
          {(user?.role === "admin" || user?.role === "superadmin") && (
            <button onClick={downloadBackup} disabled={backupLoading} style={{ width: "100%", padding: "9px 14px", background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.12)", borderRadius: 10, color: "#10b981", fontSize: 11, fontWeight: 600, cursor: backupLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 6, opacity: backupLoading ? 0.6 : 1 }}>{backupLoading ? "⏳ Generando..." : "💾 Exportar Backup"}</button>
          )}
          {user?.role === "superadmin" && (
            <>
              <input type="file" accept=".json" ref={importFileRef} onChange={handleImportBackup} style={{ display: "none" }} />
              <button onClick={() => importFileRef.current?.click()} disabled={importLoading} style={{ width: "100%", padding: "9px 14px", background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.12)", borderRadius: 10, color: "#818cf8", fontSize: 11, fontWeight: 600, cursor: importLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 6, opacity: importLoading ? 0.6 : 1 }}>{importLoading ? "⏳ Importando..." : "📥 Importar Backup"}</button>
            </>
          )}
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {filteredRepairs.map((repair, i) => {
                ...
                const repairNotes = parsedAll.notes; const repairServices = parsedAll.services; const repairSoftware = parsedAll.software; const repairRepuestos = parsedAll.repuestos; const repairDeliveryNotes = parsedAll.deliveryNotes; const repairDiscount = parsedAll.discount;
                return (
                  <div key={repair.id} className="repair-card" onClick={() => setExpandedId(isExpanded ? null : repair.id)} style={{ background: "var(--bg-card)", borderRadius: 16, border: `1px solid ${isExpanded ? st.color + "30" : "var(--border)"}`, cursor: "pointer", transition: "all 0.25s", animation: `fadeIn 0.3s ease-out ${i * 0.04}s both`, overflow: "hidden", position: "relative" }}>
                    {!isExpanded && (
                      <div className="card-compact" style={{ display: "flex", alignItems: "stretch" }}>
                        ...
                        <div className="card-info" style={{ flex: 1, padding: "12px 16px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 6, minWidth: 0 }}>
                          ...
                          <div className="filter-btns" style={{ display: "flex", gap: 4, flexWrap: "wrap", alignItems: "center" }}>
                            ...
                            {repairSoftware.map(name => <span key={name} style={{ fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 5, background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.12)", color: "#8b5cf6" }}>🎮 Soft: {name}</span>)}
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 10, minWidth: 0 }}>
                            ...
                            <div style={{ background: "var(--bg-tertiary)", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(168,85,247,0.15)", borderLeft: "3px solid #a855f7" }}>
                              ...
                              <div style={{ padding: 10, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 6 }}>
                                ...
                                {repairSoftware.length > 0 && (<div style={{ padding: "8px 10px", background: "var(--bg-hover)", borderRadius: 8, border: "1px solid var(--border)" }}><div style={{ fontSize: 8, color: "#8b5cf6", textTransform: "uppercase", fontWeight: 700, letterSpacing: "0.3px", marginBottom: 4 }}>🎮 Software</div><div className="filter-btns" style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{repairSoftware.map((name) => (<span key={name} style={{ padding: "2px 7px", background: "rgba(139,92,246,0.1)", borderRadius: 4, fontSize: 9, fontWeight: 600, color: "#8b5cf6" }}>{name}</span>))}</div></div>)}
