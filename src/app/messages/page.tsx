...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    if (_u.role === "superadmin") {
      apiFetch("/api/branches").then(r => r.json()).then(b => { if (Array.isArray(b)) { setBranches(b); const ab = localStorage.getItem("activeBranchId"); if (ab) setActiveBranch(ab); else if (b.length > 0) { setActiveBranch(b[0].id); setActiveBranchId(b[0].id); } } }).catch(() => {});
  ...
  const loadRepairs = async (token: string) => { try { const res = await apiFetch("/api/repairs", { }); if (res.ok) setRepairs(await res.json()); } catch {} };
  const loadMessages = async (repairId: string) => { const token = localStorage.getItem("token"); try { const res = await apiFetch(`/api/messages?repairId=${repairId}`, { }); if (res.ok) setMessages(await res.json()); } catch {} };
  ...
  const sendMessage = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      const res = await apiFetch("/api/messages", { method: "POST", body: JSON.stringify({ repairId: selectedRepair.id, text: newMessage.trim() }) });
    ...
    <div className="main-content" style={{ minHeight: "100vh", background: "var(--bg-primary)", paddingLeft: 200, paddingTop: 0 }}>
      ...
      <aside className={`sidebar-desktop${menuOpen ? " open" : ""}`} style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 200, transition: "transform 0.3s ease", background: "rgba(12,12,18,0.95)", backdropFilter: "blur(20px)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", zIndex: 45, padding: "0 10px" }}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, overflow: "auto", padding: "4px 0" }}>
          ...
          {[...(user?.role === "tech" ? [{ label: "Mis Asignaciones", path: "/asignaciones", icon: "📋" }, { label: "Mensajes", path: "/messages", icon: "💬" }, { label: "Escáner", path: "/scanner", icon: "📷" }, { label: "Cotizaciones", path: "/quotations", icon: "🧾" }] : [{ label: "Panel Principal", path: "/dashboard", icon: "📋" }, { label: "Servicios", path: "/services", icon: "🛠️" }, { label: "Inventario", path: "/inventory", icon: "📦" }, { label: "Software", path: "/software", icon: "🎮" }, { label: "Mensajes", path: "/messages", icon: "💬" }, { label: "Escáner", path: "/scanner", icon: "📷" }, { label: "Cotizaciones", path: "/quotations", icon: "🧾" }, { label: "Extracto", path: "/extracto", icon: "📊" }, ...(user?.role === "superadmin" ? [{ label: "Usuarios", path: "/admin/users", icon: "👥" }, { label: "Sucursales", path: "/admin/branches", icon: "🏢" }, { label: "Configuración", path: "/admin/settings", icon: "⚙️" }] : [])])].map(item => ({ ...item, active: item.path === "/messages" })).map((item) => (
